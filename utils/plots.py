...
def get_surface_trace(points, decoder, latent, resolution, mc_value, is_uniform, verbose, save_ply, connected=False):
    ...
    for i, pnts in enumerate(torch.split(grid['grid_points'], 30000, dim=0)):
        if verbose:
            print('{0}'.format(i / (grid['grid_points'].shape[0] // 30000) * 100))

        latent_code = latent.unsqueeze(dim=1).expand((-1, pnts.shape[0], -1))
        v = decoder(pnts.unsqueeze(0), latent_code)
